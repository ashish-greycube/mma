...
class PayrollInterface(AccountsController):
	def __init__(self, *args, **kwargs):
		super(PayrollInterface, self).__init__(*args, **kwargs)
	...
	def on_submit(self):
		...
		try:		
			journalentry=self.make_journal_entry(source_name=self.name)
			je_link = get_link_to_form("Journal Entry", journalentry.name)
			frappe.msgprint(_("Journal Entry {0} is created").format(je_link))		
